...
- [Bug Tracker](#bug-tracker)
  ...
  + [Session 3 (Validation Matrix Testing — 2026-07-14)](#session-3-validation-matrix-testing--2026-07-14)
    - [1. HTML Anchor Hover Inconsistency — todo](#1-html-anchor-hover-inconsistency--todo)
    ...
    - [3. GFM Collision Suffix Ambiguity (The "Commands" Problem) — todo](#3-gfm-collision-suffix-ambiguity-the-commands-problem--todo)
    - [4. Passthrough Links Produce No Debug Output — resolved](#4-passthrough-links-produce-no-debug-output--resolved)
    - [5. HTML Anchor Click Only Works in Reading Mode — in progress](#5-html-anchor-click-only-works-in-reading-mode--in-progress)
...
### 1. HTML Anchor Hover Inconsistency — todo

> **Tasks:** [TASK-0403](tasks.md#task-0403-update-triggerhover-link-interceptor-done) — hover-link interceptor, [TASK-0802](tasks.md#task-0802-toggle-setting-for-html-anchors-todo) — HTML anchor toggle. **Validation:** [`hover-preview.md`](validation/hover-preview.md).
...
### 3. GFM Collision Suffix Ambiguity (The "Commands" Problem) — todo

> **Tasks:** [TASK-0103](tasks.md#task-0103-build-document-index-in-srcdocument-indexts-done) — `buildDocumentIndex`. **Validation:** [`click-navigation.md`](validation/click-navigation.md). **Deferred to v2.**

- **Issue**: The GFM collision suffix system creates fundamental ambiguity when a heading's literal text matches the collision-suffix pattern of another heading.
...
- **Root cause**: `buildDocumentIndex` uses `index.set(finalSlug, target)` — `Map.set()` overwrites. The LAST heading to produce a given slug wins. This means `#commands-1` always goes to whichever heading produces that slug last, not first.
- **Is this a bug?**: Yes — this is arguably wrong. GFM convention (and the expected behavior) is that the FIRST heading with a given slug should be the target, with subsequent duplicates getting collision suffixes. The last-write-wins behavior of Map means that if a literal "Commands-1" heading appears BEFORE duplicate "Commands" headings, the duplicate "Commands" gets slug `commands-2` (correct). But if the literal "Commands-1" appears AFTER, the duplicate overwrites the literal's entry. Additionally, if the literal "Commands-1" heading is DELETED, `#commands-1` would silently shift to whatever heading now occupies that slug — a silent semantic change with no warning.
- **Mitigation**: This is a known GFM limitation, but our last-write-wins behavior makes it worse. A fix would be to NOT overwrite existing slug entries (check `index.has(finalSlug)` before `index.set()`), giving first-occurrence priority which matches user expectations. Documented for v2.
...
### 5. HTML Anchor Click Only Works in Reading Mode — in progress

> **Tasks:** [TASK-0803](tasks.md#task-0803-investigate--fix-html-anchor-click-in-sourcelive-preview-todo) — investigation, [TASK-0401](tasks.md#task-0401-update-openlinktext-interceptor-done) — openLinkText, [TASK-0301](tasks.md#task-0301-implement-srcreveal-targetts-done) — revealTargetInView. **Validation:** [`click-navigation.md`](validation/click-navigation.md), [`reveal-target.md`](validation/reveal-target.md).
